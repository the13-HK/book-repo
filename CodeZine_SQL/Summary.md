...
- 閉包性とは、ある集合に対してある演算を行った結果が元の集合に含まれる性質のこと。
  - 関数における入力と出力の型が同じであること。
  - Linux などのパイプライン処理も閉包性を持っている。
    - UNIX は汎ファイル主義であるため、ファイルを入力として受け取り、ファイルを出力する。
  - select 文も入力でテーブルを取り、出力をテーブルで返す関数

#### 集合のカテゴリー

集合は元来、数学に由来する概念であり、数学においては集合の種類がいくつかある。
...
- 環
  ...
  - 整数はこれにあたる
- 体
  - 加算・減算・乗算・除算について閉じている集合
  - 整数はこれにあたらない。1÷2 をすると小数になり、整数ではなくなる。
  - 有理数はこれにあたる。

### アドレス、この巨大な怪物

関係型データベースではユーザはポインタを意識することなくデータを扱うことができる。

- Oracle の rowid や PostgreSQL の oid はポインタの一種である。

  - これはあくまで参照するのみなのでそこまで気にするものでもない。

- 関係型データベースでなくてもアドレスを排除する言語は存在する。
  - それらの言語と大きく違うのは関係型データベースでは変数を使用しないこと。
  - 関係型データベースは関数型言語
...
- 両方ともテーブルで指定されたキーで分割を行うための機能。
- GROUP BY は分割後に集約して 1 行にまとめる操作が入る。
- これらは数学的にいう類と呼ぶ部分集合を作る操作である。
- 類の特徴は以下三つ
  - いずれも空集合ではない
  - 全ての部分集合の和が分割する前の集合と一致する
  - 互いに異なる任意の二つの部分集合が共通部分を持たない
- MOD も類の一種である。

### 手続き型から宣言型・集合指向へ頭を切り替える 7 箇条

- 手続き型から非手続き型への切り替えで重要なことは順序から集合への切り替えである。
  - 代入・分岐・ループが基本的な処理単位なのが手続き型言語
  - 複雑な物は単純なものの組み合わせという還元論的な考えが順序
  - 集合的な考え（SQL）は全体論的な考え方である。

#### IF 文や CASE 文は CASE 式に置き換える

- 文単位で分岐を行うのではなく、式単位で分岐を行うようにする

#### ループ処理は GROUP BY とウインドウ関数で置き換える

#### テーブルの行に順序はない

### SQL における 存在 の 階層

- SQL 文がどのレベルの階層の集合を表す式なのかを見極めることで、SQL 文の意味を理解することができる。
- GROUP BY を行うと 1 レベル上の集合を表す式になる。
  - そのため粒粒のデータを参照するようなことはできずエラーになる。
    - 例） SELECT team, AVG( age), age FROM Teams GROUP BY team;
      - age は GROUP BY で集約されたデータを参照することができないためエラーになる。
